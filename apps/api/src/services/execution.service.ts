...
export class ExecutionService {
  ...
  async getDashboardSummary(workspaceId: string, workspaceRole?: RoleType) {
    const now = new Date();
    const msIn24h = 24 * 60 * 60 * 1000;
    const since24hAgo = new Date(now.getTime() - msIn24h);
    const since48hAgo = new Date(now.getTime() - 2 * msIn24h);
    const since7dAgo = new Date(now.getTime() - 7 * msIn24h);
    const since8dAgo = new Date(now.getTime() - 8 * msIn24h);

    const workspaceObjectId = this.toObjectId(workspaceId);

    let workflowIdFilter: mongoose.Types.ObjectId[] | null = null;
    if (workspaceRole && workspaceRole !== 'owner') {
      const accessibleFolderIds = await getAccessibleFolderIds(workspaceId, workspaceRole, 'view');
      const accessibleSet = new Set(accessibleFolderIds.map((id) => id.toString()));
      const accessibleWorkflows = await Workflow.find({ workspaceId })
        .select('_id folderId')
        .lean();
      workflowIdFilter = accessibleWorkflows
        .filter((workflow) => !workflow.folderId || accessibleSet.has(String(workflow.folderId)))
        .map((workflow) => workflow._id);
    }

    const executionMatch: Record<string, unknown> = {
      workspaceId: workspaceObjectId,
      createdAt: { $gte: since8dAgo },
    };
    if (workflowIdFilter !== null) {
      executionMatch.workflowId = { $in: workflowIdFilter };
    }

    const recentExecutions = await Execution.find(executionMatch)
      .select('status durationMs createdAt')
      .lean();

    const summarizeWindow = (
      executions: Array<{ status: string; durationMs?: number; createdAt: Date }>,
    ) => {
      const total = executions.length;
      const completed = executions.filter((execution) => execution.status === 'completed').length;
      const failed = executions.filter((execution) => execution.status === 'failed').length;
    ...
    const last24hExecs = recentExecutions.filter(
      (execution) => new Date(execution.createdAt) >= since24hAgo,
    );
    const previous24hExecs = recentExecutions.filter((execution) => {
      const date = new Date(execution.createdAt);
      return date >= since48hAgo && date < since24hAgo;
    });
    const weekAgoExecs = recentExecutions.filter((execution) => {
      const date = new Date(execution.createdAt);
      return date >= since8dAgo && date < since7dAgo;
    });

    const last24h = summarizeWindow(last24hExecs);
    const previous24h = summarizeWindow(previous24hExecs);
    const weekAgoSummary = summarizeWindow(weekAgoExecs);

    const sparklineBucketCount = 12;
    const bucketMs = msIn24h / sparklineBucketCount;
    const executionsSpark = new Array<number>(sparklineBucketCount).fill(0);
    const failedSpark = new Array<number>(sparklineBucketCount).fill(0);
    const successRateSpark = new Array<number>(sparklineBucketCount).fill(0);
    const p95Spark = new Array<number>(sparklineBucketCount).fill(0);
    const bucketCompleted = new Array<number>(sparklineBucketCount).fill(0);
    const bucketDurations: number[][] = Array.from({ length: sparklineBucketCount }, () => []);

    for (const execution of last24hExecs) {
      const offset = now.getTime() - new Date(execution.createdAt).getTime();
      const bucketIndex = Math.min(
        sparklineBucketCount - 1,
        sparklineBucketCount - 1 - Math.floor(offset / bucketMs),
      );
      if (bucketIndex < 0) continue;
      executionsSpark[bucketIndex] += 1;
      if (execution.status === 'failed') {
        failedSpark[bucketIndex] += 1;
      }
      if (execution.status === 'completed') {
        bucketCompleted[bucketIndex] += 1;
      }
      if (typeof execution.durationMs === 'number' && execution.durationMs >= 0) {
        bucketDurations[bucketIndex].push(execution.durationMs);
      }
    }

    for (let i = 0; i < sparklineBucketCount; i += 1) {
      const total = executionsSpark[i];
      successRateSpark[i] = total > 0 ? Math.round((bucketCompleted[i] / total) * 1000) / 10 : 0;
      const durations = bucketDurations[i].sort((a, b) => a - b);
      p95Spark[i] = durations.length
        ? durations[Math.min(durations.length - 1, Math.floor(durations.length * 0.95))]
        : 0;
    }

    const workflowFilter: Record<string, unknown> = { workspaceId: workspaceObjectId };
    if (workflowIdFilter !== null) {
      workflowFilter._id = { $in: workflowIdFilter };
    }

    const workflowCounts = await Workflow.aggregate([
      { $match: workflowFilter },
      {
        $group: {
          _id: null,
          total: { $sum: 1 },
    ...
    const { total: totalWorkflows = 0, active: activeWorkflows = 0 } = workflowCounts[0] ?? {};
    ...
    return {
      ...
      previous24h,
      weekAgo: {
        successRate: weekAgoSummary.successRate,
        p95DurationMs: weekAgoSummary.p95DurationMs,
      },
      sparklines: {
        executions: executionsSpark,
        successRate: successRateSpark,
        p95Duration: p95Spark,
        failed: failedSpark,
      },
    };
  }

  async getWorkflowRecentActivity(workspaceId: string, workspaceRole?: RoleType) {
    const workspaceObjectId = this.toObjectId(workspaceId);
    ...
    const workflows = await Workflow.find({ workspaceId, isTemplate: { $ne: true } })
      .select('_id name status folderId lastExecutedAt updatedAt')
      .sort({ lastExecutedAt: -1, updatedAt: -1 })
      .lean();

    const visibleWorkflows = workflows.filter((workflow) => {
      if (!accessibleFolderIdsSet) return true;
      if (!workflow.folderId) return true;
      return accessibleFolderIdsSet.has(String(workflow.folderId));
    });

    const workflowIds = visibleWorkflows.map((workflow) => workflow._id);
    if (workflowIds.length === 0) {
      return [];
    }

    const msIn24h = 24 * 60 * 60 * 1000;
    const since24hAgo = new Date(Date.now() - msIn24h);

    const recentExecutions = await Execution.find({
      workspaceId: workspaceObjectId,
      workflowId: { $in: workflowIds },
      createdAt: { $gte: since24hAgo },
    })
      .select('workflowId status createdAt')
      .lean();

    const totalsByWorkflow = await Execution.aggregate([
      {
        $match: {
          workspaceId: workspaceObjectId,
          workflowId: { $in: workflowIds },
    ...
    const totalsMap = new Map<string, { total: number; completed: number; failed: number }>();
    for (const entry of totalsByWorkflow) {
      totalsMap.set(String(entry._id), {
        total: entry.total,
        completed: entry.completed,
        failed: entry.failed,
      });
    ...
    const bucketMs = msIn24h / bucketCount;
    const hourlyByWorkflow = new Map<
      string,
      Array<{ total: number; completed: number; failed: number }>
    >();

    for (const execution of recentExecutions) {
      const key = String(execution.workflowId);
      let buckets = hourlyByWorkflow.get(key);
      if (!buckets) {
        ...
        hourlyByWorkflow.set(key, buckets);
      }
      const offset = Date.now() - new Date(execution.createdAt).getTime();
      const bucketIndex = Math.min(
        bucketCount - 1,
        bucketCount - 1 - Math.floor(offset / bucketMs),
      );
      if (bucketIndex < 0) continue;
      buckets[bucketIndex].total += 1;
      if (execution.status === 'completed') buckets[bucketIndex].completed += 1;
      if (execution.status === 'failed') buckets[bucketIndex].failed += 1;
    }

    return visibleWorkflows.map((workflow) => {
      const id = String(workflow._id);
      const totals = totalsMap.get(id) ?? { total: 0, completed: 0, failed: 0 };
      const hourlyBuckets =
        hourlyByWorkflow.get(id) ??
        Array.from({ length: bucketCount }, () => ({ total: 0, completed: 0, failed: 0 }));

      return {
        workflowId: id,
        workflowName: workflow.name,
        status: workflow.status,
        lastExecutedAt: workflow.lastExecutedAt ? workflow.lastExecutedAt.toISOString() : null,
        total: totals.total,
        completed: totals.completed,
        failed: totals.failed,
        successRate:
          totals.total > 0 ? Math.round((totals.completed / totals.total) * 1000) / 10 : 0,
        hourlyBuckets,
      };
    });
  }
